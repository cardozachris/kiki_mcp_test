...
import { isFrame, isLayout, isRectangle } from "./identity";
import { generateCSSShorthand } from "./common";
...
function convertSizing(
  s?:
    | HasLayoutTrait["layoutSizingHorizontal"]
    | HasLayoutTrait["layoutSizingVertical"]
) {
...
function buildSimplifiedFrameValues(
  n: FigmaDocumentNode
): FigmaLayout | { mode: "none" } {
  if (!isFrame(n)) {
    return { mode: "none" };
  }

  const frameValues: FigmaLayout = {
    mode:
  ...
  const overflowScroll: FigmaLayout["overflowScroll"] = [];
...
function buildSimplifiedLayoutValues(
  ...
  mode: "row" | "column" | "none"
): FigmaLayout | undefined {
  if (!isLayout(n)) return undefined;

  const layoutValues: FigmaLayout = { mode };
  ...
  if (
    isFrame(parent) &&
    (parent?.layoutMode === "NONE" || n.layoutPositioning === "ABSOLUTE")
  ) {
    ...
    if (n.absoluteBoundingBox && parent.absoluteBoundingBox) {
      layoutValues.locationRelativeToParent = {
        x:
          ...
          (parent?.absoluteBoundingBox?.x ?? n.absoluteBoundingBox.x),
        y:
          n.absoluteBoundingBox.y -
          (parent?.absoluteBoundingBox?.y ?? n.absoluteBoundingBox.y),
      };
    }
    return layoutValues;
  }
  ...
  if (
    isRectangle("absoluteBoundingBox", n) &&
    isRectangle("absoluteBoundingBox", parent)
  ) {
    const dimensions: {
      width?: number;
      height?: number;
